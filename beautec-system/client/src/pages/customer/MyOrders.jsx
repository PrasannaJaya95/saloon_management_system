...
const MyOrders = () => {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        let matchDate = true;
        if (filterDate) {
            const oDate = new Date(order.createdAt).toISOString().split('T')[0];
        ...
        <div className="space-y-6">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                ...
                <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
                    ...
                    <input
                        ...
                        className="bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-purple-500 transition-all custom-date-input w-full md:w-auto"
                    />
            ...
            {filteredOrders.length === 0 ? (
                <div className="p-10 rounded-3xl bg-gray-900/40 border border-white/5 text-center text-gray-400">
                    ...
                    <p>No {filterStatus === 'All' ? 'orders' : `${filterStatus.toLowerCase()} orders`} yet.</p>
                </div>
            ) : (
